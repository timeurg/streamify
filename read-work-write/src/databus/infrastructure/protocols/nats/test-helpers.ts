...
export const natsTestRequest = async (
  ...
  let output: string;
  await testClient
    .request(subject, Empty, { noMux: true, timeout, headers: header })
    .then((m) => {
...
export const natsTestSubscribe = async (
  options: ConnectionOptions,
  subject: string,
  data: string,
) => {
    ...
    for await (const m of sub) {
      ...
      if (m.headers) {
        ...
        transactions[transactionId] =
          transactions[transactionId] || data.split('');
        m.respond(
          sc.encode(
            transactionId ? transactions[transactionId].splice(0, 1) : data,
          ),
        );
      }
    }
...
export const getConfig = async (
  ...
  const filename = path.join(
    os.tmpdir(),
    'nats_adapter_config_' + crypto.randomUUID(),
  );
