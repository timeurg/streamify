...
import {
  ...
  natsTestRequest,
  natsTestSubscribe,
} from './nats/test-helpers';

const exit$ = new Subject<void>();
let options,
  subject = '',
  config = '',
  testClient: NatsConnection;
describe('NatsProtocolAdaptor', () => {
    ...
    describe('connect', () => {
      it('should connect with [port/subject] initialization string', async () => {
        const protocol = new NatsProtocolAdaptor(
          `${options.port}/${crypto.randomUUID()}`,
          deps,
        );
        const state$ = firstValueFrom(
          protocol.state().pipe(takeUntil(exit$), timeout(1000)),
        );
  ...
  describe('integration', () => {
    test('data written to NatsProtocolAdaptor in output mode can be read from NatsProtocolAdaptor in input mode', async () => {
      ...
      const sin = input.getStream('input') as NatsReadableStream,
        sout = output.getStream('output') as NatsWritableStream;
      sout.write(data1);
      await new Promise<void>((resolve) => setTimeout(() => resolve(), 2000));
      sout.write('\n');
      sout.write(data2);
